...
/// - Parameters:
///   - distribution: Value between -1 & +1. -1 has distribution skewed to low end, +1 skews distribution to high end & 0 gives flat distribution
...
func randomValue(distribution d: CGFloat = 0, min: CGFloat = 0, max: CGFloat = 1) -> CGFloat {
    assert((-1...1).contains(d), "\n\nERROR: distribution in 'randomValue' MUST be in range -1 to +1!")         //Error if d not in range
...
//see https://stackoverflow.com/questions/75661798/function-to-change-random-distribution-in-swift by Rob Napier re above
...
/// - Parameters:
///   - d: Value between -1 & +1. -1 skews distribution to low end, +1 skews distribution to high end & 0 gives flat distribution. Default = 0
///   - min: Low end of Random range, default = 0
///   - max: High end of Random range, default = 1
/// - Returns: Random Number between 'min' & 'max'
func testRun(distribution d: CGFloat, min: CGFloat = 0, max: CGFloat = 1) {
